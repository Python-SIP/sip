...
def output_pyi(spec, pyi_filename):
    ...
    with open(pyi_filename, 'w', encoding='UTF-8') as pf:
        # Write the header.
        version_info_s = f'#\n# Generated by SIP {SIP_VERSION_STR}\n' if spec.bindings.project.version_info else ''
...
''')
    ...
    if project.sip_module:
        ...
        pf.write(f'import {project.sip_module}\n')
...
def _class(pf, spec, klass, defined, indent=0):
    ...
    if not klass.is_hidden_namespace:
        ...
        if klass.superclasses:
            ...
            if project.abi_version[0] == 12 and project.sip_module and klass.supertype.name.startswith('sip.'):
                s += project.sip_module + klass.supertype.name[4:]
            else:
